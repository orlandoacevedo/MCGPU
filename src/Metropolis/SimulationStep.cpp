...
Real SimulationStep::calcMolecularEnergyContribution(int currMol,
                                                     int startMol,
                                                     SimBox* box) {
...
Real SimulationStep::calcSystemEnergy(Real &subLJ, Real &subCharge,
  ...
  for (int mol = 0; mol < box->numMolecules; mol++) {
    total += calcMolecularEnergyContribution(mol, mol, box);
  }
