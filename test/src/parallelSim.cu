...
__global__ void calcEnergy(ParallelSim *sim,Atom *atoms, Environment *enviro, double *energySum, DeviceMolecule *dev_molecules, Hop *hops){
    ...
    if(idx < N ){
        ...
        xAtom_pos = sim->getXFromIndex(idx);
        yAtom_pos = sim->getYFromIndex(xAtom_pos, idx);
        ...
        if(xAtom.sigma < 0 || xAtom.epsilon < 0 || yAtom.sigma < 0 || yAtom.epsilon < 0){
            ...
            lj_energy = sim->calc_lj(xAtom,yAtom,*enviro);
            charge_energy = sim->calcCharge(xAtom, yAtom, enviro);
            double fValue = 1.0;
            if (dev_molecules != NULL){
               fValue = sim->getFValue(xAtom, yAtom, dev_molecules, enviro, hops);
            }
...
__device__ int ParallelSim::getXFromIndex(int idx){
    int c = -2 * idx;
...
__device__ int ParallelSim::getYFromIndex(int x, int idx){
    return idx - (x * x - x) / 2;
}

__device__ double ParallelSim::makePeriodic(double x, double box){
    while(x < -0.5 * box){
        x += box;
    }
...
__device__ double ParallelSim::calc_lj(Atom atom1, Atom atom2, Environment enviro){
...
__device__ double ParallelSim::calcCharge(Atom atom1, Atom atom2, Environment *enviro){
...
__device__ double ParallelSim::calcBlending(double d1, double d2){
    return sqrt(d1 * d2);
}

__device__ int ParallelSim::getMoleculeFromAtomID(Atom a1, DeviceMolecule *dev_molecules, Environment enviro){
    int atomId = a1.id;
    int currentIndex = enviro.numOfMolecules - 1;
...
__device__ double ParallelSim::getFValue(Atom atom1, Atom atom2, DeviceMolecule *dev_molecules, Environment *enviro, Hop *hops){
    GPUSimBox *box=getdevGPUSimBox();
...
/*        Hop *molecHops = (Hop *)malloc(molecHopSize);
        ...
        int hopChain = hopGE3(atom1.id, atom2.id, dev_molecules[moleculeIndex], box->getdevHop());
//        free(molecHops);
        if (hopChain == 3)
            return 0.5;
        else if (hopChain > 3)
...
__device__ int ParallelSim::hopGE3(int atom1, int atom2, DeviceMolecule dev_molecule,Hop *molecule_hops){
    Hop myHop;
