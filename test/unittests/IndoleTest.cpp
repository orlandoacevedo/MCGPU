...
TEST(IndoleTest, FrontToEndIntegrationTest)
{
  ...
  ss << MCGPU << "/bin/metrosim "
       ...
       << MCGPU << "/test/unittests/Integration/IndoleTest/IndoleTest.config "
                   "-s --name indoleCPU -k > /dev/null";


    // Launch MCGPU application in serial, expect output files in /MCGPU/ directory   
  system(ss.str().c_str());

  double expected = 1832460;
  double energyResult = getEnergyResult("indoleCPU.results");
  // Clean up
  remove(std::string(MCGPU +
                     "test/unittests/Integration/IndoleTest/IndoleTest.config"
                     ).c_str());
