 ...
 * the config file.
 */
void createIndole4000ConfigFile(std::string MCGPU, std::string fileName,
                                std::string primaryAtomIndexString) {
  ConfigFileData settings = ConfigFileData(
      87.17, 87.17, 87.17, 298.15, .06, 100000, 4000,
      "resources/bossFiles/oplsaa.par",
      "test/unittests/MultipleSolvents/indole4000Tests/indole.z",
      "test/unittests/MultipleSolvents/indole4000Tests", 12.0, 6.0, 12345);
 ...
 */
std::string buildIndole4000Command(std::string MCGPU, std::string configFile,
  ...
  return buildCommand(MCGPU, configFile, outputName, series, neighborlist,
                      errorExpected,
                      "test/unittests/MultipleSolvents/indole4000Tests");
}
...
TEST(Indole4000Test, OnePrimaryIndex)
{
  ...
  system(buildIndole4000Command(MCGPU, "indole4000-1MPI.config",
                                "indole4000-1MPI", true, false, false).c_str());
  double expected = 47518400;
  double energyResult = getEnergyResult("indole4000-1MPI.results");
...
TEST(Indole4000Test, OnePrimaryIndex_GPU)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000-1MPI.config",
                                "indole4000-1MPI-GPU", false, false,
                                false).c_str());
  double expected = 47518400;
  double energyResult = getEnergyResult("indole4000-1MPI-GPU.results");
  EXPECT_NEAR(expected, energyResult, 100);
}
#endif
...
TEST(Indole4000Test, NeighborListFunction1MPI)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000-1MPI.config",
                                "indole4000-1MPI-NL", true, true,
                                false).c_str());
  double expected = 47518400;
  double energyResult = getEnergyResult("indole4000-1MPI-NL.results");
  EXPECT_NEAR(expected, energyResult, 100);
}
...
TEST(Indole4000Test, NeighborListFunction1MPI_GPU)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000-1MPI.config",
                                "indole4000-1MPI-NL-GPU", false, true,
                                false).c_str());
  double expected = 47518400;
  double energyResult = getEnergyResult("indole4000-1MPI-NL-GPU.results");
...
TEST(Indole4000Test, TwoPrimaryIndex)
{
  ...
  system(buildIndole4000Command(MCGPU, "indole4000-2MPI.config",
                                "indole4000-2MPI", true, false,
                                false).c_str());
  double expected = 47463200;
  double energyResult = getEnergyResult("indole4000-2MPI.results");
  EXPECT_NEAR(expected, energyResult, 100);
}
...
TEST(Indole4000Test, TwoPrimaryIndex_GPU)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000-2MPI.config",
                                "indole4000-2MPI-GPU", false, false,
                                false).c_str());
  double expected = 47463200;
  double energyResult = getEnergyResult("indole4000-2MPI-GPU.results");
  EXPECT_NEAR(expected, energyResult, 100);
}
#endif
...
TEST(Indole4000Test, NeighborListFunction2MPI)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000-2MPI.config",
                                "indole4000-2MPI-NL", true, true,
                                false).c_str());
    double expected = 47463200;
    double energyResult = getEnergyResult("indole4000-2MPI-NL.results");
    EXPECT_NEAR(expected, energyResult, 100);
}
...
TEST(Indole4000Test, NeighborListFunction2MPI_GPU)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000-2MPI.config",
                                "indole4000-2MPI-NL-GPU", false, true,
                                false).c_str());
    double expected = 47463200;
    double energyResult = getEnergyResult("indole4000-2MPI-NL-GPU.results");
    EXPECT_NEAR(expected, energyResult, 100);
}
#endif
...
TEST(Indole4000Test, MultipleSolventsDefinition)
{
  ...
  system(buildIndole4000Command(MCGPU, "indole4000MulSolvent.config",
                               "indole4000MulSolvent.txt", true, false,
                               true).c_str());
...
TEST(Indole4000Test, MultipleSolventsDefinition_GPU)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000MulSolvent.config",
                                "indole4000MulSolvent-GPU.txt", false, false,
                                true).c_str());
  std::string errorResult = getErrorResult(MCGPU,
                                           "indole4000MulSolvent-GPU.txt");
  EXPECT_STREQ("loadBoxData()", errorResult.c_str());
}
#endif
...
{
  std::string MCGPU = getMCGPU_path();
  createIndole4000ConfigFile(MCGPU, "indole4000MulSolvent-MPI.config",
                             "[1,2],[3,4]");
  system(buildIndole4000Command(MCGPU, "indole4000MulSolvent-MPI.config",
                                "indole4000MulSolvent-MPI.txt", true, false,
                                true).c_str());
  std::string errorResult = getErrorResult(MCGPU,
                                           "indole4000MulSolvent-MPI.txt");
  EXPECT_STREQ("loadBoxData()", errorResult.c_str());
}
...
TEST(Indole4000Test, MultipleSolventsDefinitionMPI_GPU)
{
  std::string MCGPU = getMCGPU_path();
  system(buildIndole4000Command(MCGPU, "indole4000MulSolvent-MPI.config",
                                "indole4000MulSolvent-MPI-GPU.txt", false,
                                false, true).c_str());
  std::string errorResult = getErrorResult(MCGPU,
                                           "indole4000MulSolvent-MPI-GPU.txt");
  EXPECT_STREQ("loadBoxData()", errorResult.c_str());
}
#endif
...
{
  std::string MCGPU = getMCGPU_path();
  createIndole4000ConfigFile(MCGPU, "indole4000SingleMultipleIndexes.config",
                             "1,[1,2]");
  system(buildIndole4000Command(MCGPU,
                                "indole4000SingleMultipleIndexes.config",
                                "indole4000SingleMultipleIndexes.txt", true,
                                false, true).c_str());
  std::string errorResult = getErrorResult(MCGPU,
                                           "indole4000SingleMultipleIndexes.txt");
  EXPECT_STREQ("loadBoxData()", errorResult.c_str());
}

// Test indole with 4000 molecules and multiple solvent primary indexes of
// 1,[1,2] on GPU
#ifdef _OPENACC
TEST(Indole4000Test, SingleMultipleIndexes_GPU)
{
  ...
  system(buildIndole4000Command(MCGPU,
                                "indole4000SingleMultipleIndexes.config",
                                "indole4000SingleMultipleIndexes-GPU.txt",
...
{
  std::string MCGPU = getMCGPU_path();
  createIndole4000ConfigFile(MCGPU, "indole4000SingleMultipleIndexes2.config",
                             "[1,2],1");
  system(buildIndole4000Command(MCGPU,
                                "indole4000SingleMultipleIndexes2.config",
                                "indole4000SingleMultipleIndexes2.txt", true,
                                false, true).c_str());
...
TEST(Indole4000Test, SingleMultipleIndexes2_GPU)
{
  ...
  system(buildIndole4000Command(MCGPU,
                                "indole4000SingleMultipleIndexes2.config",
                                "indole4000SingleMultipleIndexes2-GPU.txt",
